...
The terms are sorted by the number of service orders they identify (column one). Column six displays *p*-values for means tests across samples. It compares whether the sample identified in any row, whose search pattern takes in all terms up to, and inclusive of, that row, is significantly different from the the sample identified by all search terms up to the row before. Row two, for instance 
